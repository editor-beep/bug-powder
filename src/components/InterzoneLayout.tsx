...
export function InterzoneLayout() {
  ...
  return (
    <div className="iz-scanlines iz-noise min-h-dvh w-full max-w-full overflow-x-hidden">
      <header className="sticky top-0 z-50 backdrop-blur-sm bg-iz-ink/85 border-b border-iz-vein">
        <div className="max-w-7xl mx-auto px-3 sm:px-4 py-3 flex items-center justify-between gap-3 sm:gap-4 min-w-0">
          <Link to="/feed" className="flex min-w-0 items-center gap-2 group">
            <Skull className="w-5 h-5 text-iz-blood group-hover:text-iz-pus transition-colors iz-flicker" />
            <span className="font-display text-base sm:text-lg text-iz-bone iz-glitch-text tracking-wider truncate">
              INTERZONE_FEED
            </span>
          ...
          <nav className="hidden md:flex items-center gap-1">
            {navItems.map((item) => (
              <Link
                ...
                activeProps={{
                  className:
                    "px-3 py-1 text-xs uppercase tracking-widest text-iz-bone border border-iz-blood bg-iz-vein/40",
                }}
              >
                {item.label}
              </Link>
          ...
          <div className="flex shrink-0 items-center gap-2 sm:gap-3 text-[10px] font-mono">
            <span className="hidden sm:inline text-muted-foreground">
              DESCENT <span className="text-iz-pus">{descent}</span>
            </span>
            <span className="hidden sm:inline text-muted-foreground">
              STREAK <span className="text-iz-blood">{streak}</span>
            </span>
            <span className="hidden md:inline text-muted-foreground">
              PARANOIA <span className="text-iz-pus">{paranoia}</span>
            </span>
            <button
            ...
            >
              {audioEnabled ? (
                <Volume2 className="w-3.5 h-3.5 text-iz-pus" />
              ) : (
        ...
        <nav className="md:hidden border-t border-iz-vein flex">
          {navItems.map((item) => (
            <Link
              ...
              activeProps={{
                className:
                  "flex-1 text-center py-2 text-[10px] uppercase tracking-widest text-iz-pus bg-iz-vein/30",
              }}
            >
              {item.label}
            </Link>
      ...
      <main className="max-w-7xl mx-auto px-3 sm:px-4 py-6 min-w-0">
        <Outlet />
      </main>
    </div>
  );
